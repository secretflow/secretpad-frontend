...
export class DefaultParametersModificationService extends Model {
  /** 分箱数据 */
  binningDatas?: ParametersData[];

  componentConfigService = getModel(DefaultComponentConfigService);

  undoService = getModel(DefaultUndoService<ParametersData>);

  private convertToBinningConfig = (binningData: ParametersData, node: NodeAllInfo) => {
   ...
   */
  public saveBinningConfig = async (binningData: ParametersData, node: NodeAllInfo) => {
   ...
   */
  public updateBinningConfig = async (
    binningData: ParametersData,
    node: NodeAllInfo,
  ) => {
    const config = this.convertToBinningConfig(binningData, node);
